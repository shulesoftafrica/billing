<section class="api-section" id="subscriptions-section">
    ...
    <x-docs.endpoint id="list-subscriptions" method="GET" url="/api/v1/subscriptions" title="List Subscriptions"
    ...
    <x-docs.endpoint id="create-subscription-invoice" method="POST" url="/api/v1/invoices"
        ...
        <x-slot name="responses">
            ...
            <div class="mt-3">
                ...
                <ul>
                    ...
                    <li>Control numbers and payment links are generated based on <code>payment_gateway</code> parameter
                    </li>
                    <li><strong>Idempotent:</strong> If a pending subscription already exists for the same customer and
                        price plan, existing invoice is returned</li>
                </ul>
            </div>
        </x-slot>
    </x-docs.endpoint>

    <x-docs.endpoint id="get-subscription" method="GET" url="/api/v1/subscriptions/{id}" title="Get Subscription"
...
{{-- 
    <h3 style="margin-top: 40px; color: #333; border-bottom: 2px solid #e0e0e0; padding-bottom: 10px;">⬆️ Subscription
        Upgrade & Downgrade</h3>
    <p>Change subscription plans mid-cycle with automatic proration. The system calculates fair charges based on actual
        billing cycle days.</p>

    <x-docs.endpoint id="upgrade-subscription" method="POST" url="/api/v1/invoices/plan-upgrade"
        ...
        <x-slot name="requestBody">
            ...
            <x-docs.parameter-table :headers="['Parameter', 'Type', 'Required', 'Description']">
                ...
                <tr>
                    ...
                    <td>Payment method: <code>flutterwave</code>, <code>control_number</code>, or <code>both</code>
                        (default: both)</td>
                </tr>
                <tr>
            ...
            <div class="alert alert-info"
                style="background: #e3f2fd; border-left: 4px solid #2196F3; padding: 15px; margin: 20px 0;">
...
Amount to Pay = New Plan Charge - Unused Credit</pre>
            ...
            <div class="alert alert-success"
                style="background: #e8f5e9; border-left: 4px solid #4caf50; padding: 15px; margin: 20px 0;">
        ...
        <x-slot name="responses">
            ...
            <x-docs.code-block language="json">
                {
                "success": false,
                "message": "Failed to upgrade subscription: New plan must have a higher price than current plan. Use
                downgrade endpoint for lower-tier plans."
                }
            </x-docs.code-block>
        </x-slot>
    </x-docs.endpoint>

    <x-docs.endpoint id="downgrade-subscription" method="POST" url="/api/v1/invoices/plan-downgrade"
        ...
        <x-slot name="requestBody">
            ...
            <div class="alert alert-info"
                style="background: #fff3e0; border-left: 4px solid #ff9800; padding: 15px; margin: 20px 0;">
...
Credit Amount = Unused Value - New Plan Cost</pre>
        ...
        <x-slot name="responses">
            ...
            <x-docs.code-block language="json">
                {
                "success": false,
                "message": "Failed to downgrade subscription: New plan must have a lower price than current plan. Use
                upgrade endpoint for higher-tier plans."
                }
            </x-docs.code-block>
        </x-slot>
    </x-docs.endpoint>
    ...
    <div class="alert alert-warning">
        <h4>Important Business Rules</h4>
        <ul>
            <li><strong>Subscription Status:</strong> Only <code>active</code> subscriptions can be upgraded or
                downgraded</li>
            <li><strong>Same Product:</strong> New plan must belong to the <strong>same product</strong> as current plan
            </li>
            ...
            <li><strong>Proration:</strong> All calculations use <strong>actual billing cycle days</strong> (28-31 days
                depending on month)</li>
    ...
    <x-docs.endpoint id="cancel-subscription" method="POST" url="/api/v1/subscriptions/{id}/cancel"
        title="Cancel Subscription" description="Cancel an inactive or pending subscription. You cannot cancel active subscriptions with billing cycles.">
