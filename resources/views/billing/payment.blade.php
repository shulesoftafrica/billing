...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .total-due-date { text-align: right; }
...
@php
    $currencyCode = strtoupper((string) ($invoice->currency ??  'TZS'));
@endphp
            ...
            <div class="invoice-meta">
                ...
                <div class="invoice-dates">
                    <div class="date-item">
                        ...
                        <div class="date-value">{{ $invoice->issued_at ? \Carbon\Carbon::parse($invoice->issued_at)->format('M d, Y') : '-' }}</div>
                    </div>
                    <div class="date-item">
                        <div class="date-label">Due date</div>
                        <div class="date-value">{{ $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('M d, Y') : '-' }}</div>
                    </div>
                </div>
                ...
                <div class="line-items">
                    ...
                    <div class="line-item">
                        <div class="item-info">
                            <div class="item-name">{{ $item->pricePlan?->name ?? 'Invoice Item' }}</div>
                            <div class="item-desc">Qty {{ number_format((float) $item->quantity, 2) }} × {{ $currencyCode }} {{ number_format((float) $item->unit_price, 2) }}</div>
                        </div>
                        <div class="item-price">
                            {{ $currencyCode }} {{ number_format((float) $item->total, 2) }}
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="total-block">
            <div>
                ...
                <div class="total-amount">
                    <span class="total-currency">{{ $currencyCode }}</span>{{ number_format((float) $invoice->total, 2) }}
                </div>
            </div>
            <div class="total-due-date">
                <div class="due-label">Pay by</div>
                <div class="due-date">{{ $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('M d, Y') : '-' }}</div>
            </div>
        </div>
    </div>
            ...
            <button class="pay-btn" id="pay-btn" type="button">
                <div class="btn-inner">
                    ...
                    <span class="btn-label">
                        Pay {{ $currencyCode }} {{ number_format((float) $invoice->total, 2) }}
                    </span>
                </div>
            </button>
